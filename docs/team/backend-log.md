...
| P1 | 序列化接口规格书（待积累更新：无尽模式 + critDmgBonus/goldDropBonus + 9新协同 + BoomerangSnap + 毒雾poison + 2新敌人 + 4关卡敌人 + 2新Boss + stageId + secrets + skills面板 + Weapon Mastery + Pet/伙伴系统 + Ultimate系统） | ⚠️ Drive #50 里程碑巡检：28 项累积更新待合并，阻塞于前端 Player 拆分 |
| P1 | Player 拆分方案设计（LocalPlayer / RemotePlayer 接口抽象） | 🔨 设计完成待评审，Drive #50 确认前端仍未启动 |
