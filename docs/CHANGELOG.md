...
### QA
- Drive #28 测试 14/14 通过（全绿，4.5分钟）
- quests_all 成就硬编码修复验证：发现前端 commit 仅记录日志未修改代码，硬编码值(14)与 CFG.QUESTS.length 一致，当前无功能影响
- 技术债务记录：quests_all `>= 14` 和 quests_half `>= 7` 应改为动态读取数组长度
- 连续8个Drive零回归（Drive #20~#28）
- 全部缺陷已关闭（BUG-001~013 + ENH-001/002）
...
### Frontend
- 进化武器雷霆回旋 (Thunderang) 实现 -- 回旋镖+闪电进化
  - 追踪4枚回旋镖(75度偏转角)，每次命中40%概率触发闪电链(120px/2目标/8伤害/2层/50%衰减)
  - 双程闪电判定：飞出和飞回各独立判定闪电链
  - 蓝金配色V字形+电弧缠绕视觉
- 进化武器烈焰回旋 (Blazerang) 实现 -- 回旋镖+火焰法杖进化
  - 3枚回旋镖沿弧线飞行，飞行路径每20px留下火焰轨迹点(持续1.5秒)
  - 命中敌人点燃2.5秒(3DPS)，火焰轨迹面积伤害(2DPS)
  - 最大20个火焰轨迹点，橙红色V字形+火焰轨迹视觉
- 进化系统从6条路线扩展至8条路线

### QA
- Drive #21 测试 14/14 通过（全绿，4.5分钟）
- 4个变更文件 JS 语法检查全部通过
- Thunderang/Blazerang 代码审查完成（CFG常量/飞行逻辑/特殊效果/视觉渲染/进化注册）
- 发现 BUG-010 (Low): all_evolutions 成就 parts 遗漏新增2种进化武器
